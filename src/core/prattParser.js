...
export class PrattParser {
    ...
    eat(tokenType) {
        ...
        if (token == null)
            throw new SyntaxError(`Fim inesperado da expressão.`);
        if (token.type !== tokenType)
            throw new SyntaxError(`Esperado ${tokenType}, recebido ${token.type}`);
        this.lookahead = this.tokenizer.getNextToken();
        return token;
    }
    parseExpression(precedence = 0) {
        ...
        if (!token)
            throw new SyntaxError(`Expressão vazia.`);
        this.lookahead = this.tokenizer.getNextToken();
    ...
    nud(token) {
        ...
        if (token.type === TokenTypes.SUBTRACTION)
            return ["Negate", this.parseExpression(14)];
    ...
    led(token, left) {
        if (token.type === TokenTypes.ADDITION)
            return ["Add", left, this.parseExpression(10)];
        if (token.type === TokenTypes.SUBTRACTION)
            return ["Subtract", left, this.parseExpression(10)];
        if (token.type === TokenTypes.MULTIPLICATION)
            return ["Multiply", left, this.parseExpression(20)];
        if (token.type === TokenTypes.DIVISION)
            return ["Divide", left, this.parseExpression(20)];
        if (token.type === TokenTypes.EXPONENTIATION)
            return ["Power", left, this.parseExpression(30 - 1)];
        throw new SyntaxError(`Infixo desconhecido para: ${token.value}`);
    }
    getPrecedence(type) {
        if (type === TokenTypes.ADDITION || type === TokenTypes.SUBTRACTION)
            return 10;
        if (type === TokenTypes.MULTIPLICATION || type === TokenTypes.DIVISION)
            return 20;
        if (type === TokenTypes.EXPONENTIATION)
            return 30;
        return 0;
    }
}
